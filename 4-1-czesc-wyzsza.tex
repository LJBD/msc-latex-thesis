...
Ogólna forma funkcjonału celu obsługiwanego przez ACADO jest dana równaniem \ref{eq:acado-quality}, która składa się z wyrażenia podcałkowego Lagrange'a $L$ oraz wyrażenia Mayera $M$.

\begin{equation}\label{eq:acado-quality}
\Phi[x(\cdot), z(\cdot), u(\cdot), p, T] = \int_{t_{0}}^{T}L(\tau, x(\tau), z(\tau), u(\tau), p, T)d\tau + M(x(T), p, T)
\end{equation}
%TODO: dokończyć ten opis po ogólnym opisie metod numerycznych rozwiązywania problemów optymalizacji dynamicznej
